...
function ptype_(vars){
	var proptype = vars.split('-');
	if(proptype[0]){
		document.getElementById('ptypeid').value = proptype[0];	
	}
	if(proptype[1]=='no'){
		for (var c=1; c<=5; c++){
			var bdt = "bdroom"+c;
			document.getElementById(bdt).disabled = true;		
		}
	} else{
		for (var c=1; c<=5; c++){
			var bdt = "bdroom"+c;
			document.getElementById(bdt).disabled = false;		
		}
	}
