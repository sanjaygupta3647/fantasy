<?php
	...
	$resultss=$cms->db_query("select url from #_matches  group by url ");
	while($ress=mysql_fetch_array($resultss)){ extract($ress); 
		$matchurl[] = $url; 
	}
	$newss=$cms->db_query("select url from #_news  group by url ");
	while($ress=mysql_fetch_array($newss)){ extract($ress); 
		$newsurl[] = $url; 
	}
	$fixurls[] = SITE_PATH;*/
?>